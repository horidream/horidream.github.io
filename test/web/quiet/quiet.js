...
var Quiet = (function () {
  ...
  function transmitter(opts) {
    ...
    if (typeof profile === "object") {
      profileObj = profile;
      c_profiles = Module.intArrayFromString(
        JSON.stringify({ profile: profileObj })
      );
    ...
    var encoder = Module.ccall(
      "quiet_encoder_create",
      "pointer",
      ["pointer", "number"],
      [opt, audioCtx.sampleRate]
    );
    ...
    if (opts.clampFrame) {
      ...
      frame_len = Module.ccall(
        "quiet_encoder_clamp_frame_len",
        "number",
        ["pointer", "number"],
        [encoder, sampleBufferSize]
      );
    } else {
      frame_len = Module.ccall(
    ...
    var writebuf = function () {
      ...
      while (true) {
        ...
        var written = Module.ccall(
          "quiet_encoder_send",
          "number",
          ["pointer", "array", "number"],
          [encoder, new Uint8Array(frame), frame.byteLength]
        );
        if (written === -1) {
      ...
      var written = Module.ccall(
        ...
        [encoder, samples, sampleBufferSize]
      );
      var after = new Date();
  ...
  function receiver(opts) {
    ...
    if (gUM === undefined) {
      ...
      if (opts.onCreateFail !== undefined) {
        opts.onCreateFail(
          "getUserMedia undefined (mic not supported by browser)"
        );
      }
      return;
    }
    ...
    var readbuf = function () {
      ...
      while (true) {
        var read = Module.ccall(
          "quiet_decoder_recv",
          "number",
          ["pointer", "pointer", "number"],
          [decoder, frame, frameBufferSize]
        );
        if (read === -1) {
          break;
        }
    ...
    var consume = function () {
      ...
      Module.ccall(
        ...
        "number",
        ["pointer", "pointer", "number"],
        [decoder, samples, sampleBufferSize]
      );
      var after = new Date();
      ...
      var currentChecksumFailCount = Module.ccall(
      ...
      if (
        opts.onReceiveFail !== undefined &&
        currentChecksumFailCount > lastChecksumFailCount
      ) {
        window.setTimeout(function () {
      ...
      if (opts.onReceiverStatsUpdate !== undefined) {
        ...
        var frames = Module.ccall(
          "quiet_decoder_consume_stats",
          "pointer",
          ["pointer", "pointer"],
          [decoder, num_frames_ptr]
        );
        ...
        for (var i = 0; i < num_frames; i++) {
          ...
          frameStats.receivedSignalStrengthIndicator =
            Module.HEAPF32[frame + 3];
  ...
  function ab2str(ab) {
    return decodeURIComponent(
      escape(String.fromCharCode.apply(null, new Uint8Array(ab)))
    );
  }
